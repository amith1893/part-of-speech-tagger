...
class ProbabilisticTagger():
    ...
    def run_viterbi(self, test_sent):
        ...
        for tag in tag_set:
            ...
            #self.viterbi[tag][0] = log(self.transition_tag_dict[start_state][tag]) + log(emission_prob_value)
        ...
        for i in range(1, len(test_words)):
            for tag in tag_set:
                ...
                #print "I chose %s" %maxm_vit_tag
                self.backtrack[tag][i] = [maxm_back_tag, i-1]

        #print "VITERBI DICTTT"
        #print self.viterbi
        maxm_vit = 0
        maxm_back_tag = 0
        for tag in tag_set:
    ...
    def run_testcase(self, test_untagged_file, viterbi_tagged_file):
        ...
        for sent in sentence_list:
            print "INPUT :\t" + str(sent[:len(sent)-1])
...
if __name__ == "__main__":
    ...
    pt.mapWordToTags("berp-POS-training.txt")
    #pt.mapTestDataToTags("berp-POS-test-untagged.txt", "berp-POS-test-tagged.txt")
